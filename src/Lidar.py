...
class LidarScanner:
    ...
    def transform_scan(self, scan):
        ...
        for i, point in enumerate(scan):
            ...
            if self.sens == "avant" and (angle < 320 and angle > 40):
                continue
            elif self.sens == "arriere" and (angle < 140 and angle > 220):
                continue
            ...
            try:
                ...
                for scan in self.lidar.iter_scans():
                    ...
                    if len(self.new_scan) > 0:  
                        # détecter les objets
                        new_objets = self.detect_objects(self.new_scan)
                        self.client_socket.add_to_send_list(self.client_socket.create_message(0, "objects", self.generate_JSON_Objets(new_objets)))
                        ...
                        """if len(new_objets) > 0:
                            ...
                            if distance_objet < 600 and self.en_mvt and self.state_robot == "move":
                            ...
                            elif (distance_objet > 600 and self.state_robot == "pause" and (time.time() - Time) > 3)  or ((time.time() - Time) > 4 and self.state_robot == "pause"):
                        ...
                        elif self.state_robot == "pause" and (time.time() - Time) > 4:
                            self.client_socket.add_to_send_list(self.client_socket.create_message(4, "lidar", {"etat": "resume"}))
                            self.state_robot = "move"
